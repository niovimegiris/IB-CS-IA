...
def index(): # main page
    ...
    if form.validate_on_submit(): # checks if user inputs are valid before retrieving data from APIs
        ...
        if venues: # checks if venues exists
            venues = randomly_select_venues(venues, 4)
            optimized_route = get_route(origin=form.origin.data,
                                        destination=form.destination.data,
                                        venues=venues
                                        travel_mode=form.travel.mode.data,
                                        return_to_start=form.back_to_start.data)
            if optimized_route:
        ...
        if venues and not optimized_route: # when venues exists but Google can't find route
            trip_plan.append('Venues found, no route found. Try another travel mode.')
